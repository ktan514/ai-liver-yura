        ...
        *,
        ...
        if session is None:
            return self._decision(
                operation, session_id, None, False, "lifecycle.stale_session", True, trace_id
            )
        active = self._sessions.find_active_or_preparing()
        if active is not None and active.session_id != session_id:
            return self._decision(
                operation, session_id, session, False, "lifecycle.stale_session", True, trace_id
            )
        if expected_version is not None and session.state_version != expected_version:
            return self._decision(
                operation, session_id, session, False, "lifecycle.version_mismatch", False, trace_id
            )
        ...
        if category == StreamLifecycleClass.TERMINAL:
            return self._decision(
                operation, session_id, session, False, "lifecycle.terminal", False, trace_id
            )
        ...
        if operation == LifecycleOperation.START_NORMAL_END:
            allowed = session.status in {StreamSessionStatus.LIVE, StreamSessionStatus.STOP_FAILED}
            return self._decision(
                operation,
                session_id,
                session,
        ...
        if category == StreamLifecycleClass.NORMAL_ENDING:
            return self._decision(
                operation, session_id, session, False, "lifecycle.ending", False, trace_id
            )
        if category == StreamLifecycleClass.FAILED:
            return self._decision(
                operation, session_id, session, False, "lifecycle.failed", True, trace_id
            )
        if session.status != StreamSessionStatus.LIVE:
            return self._decision(
                operation, session_id, session, False, "lifecycle.not_live", False, trace_id
            )
        ...
        if self._last_notifications.get(key) != signature:
            ...
            self._publisher(
                event,
                {
                    ...
                    "external_state_summary": dict(self._external[session_id])
