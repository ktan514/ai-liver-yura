...
class PrepareStreamSessionUsecase:
    ...
    async def _youtube_checks(self, broadcast_id: str) -> tuple[HealthCheckItem, ...]:
        ...
        auth = await self._call(
            "youtube.authentication", "youtube", required, self._youtube.check_authentication()
        )
        checks.append(
    ...
    @staticmethod
    def _from_call(
        result: _CallResult, status: HealthStatus, summary: str, failure_reason: str | None
    ) -> HealthCheckItem:
    ...
    @staticmethod
    def _condition(
        check_id: str, component: str, required: bool, condition: bool, ok: str, failure: str
    ) -> HealthCheckItem:
    ...
    def _cancel(
        self, command: StreamPreparationCommand, session: StreamSession, started_at: datetime
    ) -> StreamPreparationResult:
