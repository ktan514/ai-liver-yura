...
class YouTubeLiveChatPoller:
    ...
    async def poll_once(self) -> bool:
        ...
        self._status = replace(
            ...
            last_message_at=max(
                (item.published_at for item in unique), default=self._status.last_message_at
            ),
    ...
    async def _handle_error(self, error: Exception) -> bool:
        ...
        if retryable and self._status.attempt <= self._max_retries:
            interval = min(
                60_000, int(1000 * 2 ** max(0, self._status.attempt - 1) + random.uniform(0, 250))
            )
            self._status = replace(
                self._status,
    ...
    def _event(self, message: NormalizedLiveChatMessage) -> AgentEvent:
    ...
    def _normalize(
        ...
        return NormalizedLiveChatMessage(
            ...
            str(details.get("amountDisplayString")) if details.get("amountDisplayString") else None,
    ...
    def _classify_error(error: Exception) -> tuple[str, bool, bool]:
        ...
        if isinstance(error, YouTubeApiError):
            ...
            return mapping.get(error.kind, "live_chat.invalid_response"), error.retryable, False
