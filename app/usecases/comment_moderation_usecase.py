...
class CommentModerationUsecase:
    ...
    async def evaluate_event(self, event: AgentEvent) -> CommentModerationDecision | None:
        ...
        if self._queued >= self._settings.evaluation_queue_capacity:
            self._publish(
                ...
                {"session_id": session_id, "reason_code": "comment_moderation.queue_full"},
        ...
        if decision.status == "allow" and decision.ranking_eligible and decision.sanitized_text:
            ...
            self._candidate_sink(candidate, event.trace_context.trace_id)
            self._publish(
