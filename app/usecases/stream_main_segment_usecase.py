...
MainExecutor = Callable[[dict[str, object], str], Awaitable[ActivityTurnResult]]
...
class StreamMainSegmentUsecase:
    ...
    async def start(
        ...
        activity = StreamMainSegmentActivity(
            session.session_id, opening.trace_id, segment.segment_id, segment.order, segment.title
        )
    ...
    async def _execute(
        ...
        self._publish(
            "stream_main_segment.generation_started", self._data(activity), activity.trace_id
        )
        ...
        try:
            self._publish(
                "stream_main_segment.output_started", self._data(activity), activity.trace_id
            )
            self._publish(
                "stream_main_segment.speech_started", self._data(activity), activity.trace_id
            )
    ...
    def _missing(
        ...
        self._activities.create(activity)
        return self._fail(
            activity.transition(StreamMainSegmentStatus.RUNNING), code, retryable=retryable
        )
