...
class CommentRankingUsecase:
    ...
    async def _select(
        ...
        gate = self._gate.evaluate(
            LifecycleOperation.SELECT_COMMENT_RESPONSE_TARGET, session_id, trace_id=trace_id
        )
        if not gate.allowed:
        ...
        self._publish(
            ...
            {"session_id": session_id, "ranking_run_id": run_id, "candidate_count": len(pool)},
            trace_id,
        )
        try:
        ...
        if not self._selections.reserve(target):
            return self._not_selected(
                session_id, ("comment_ranking.duplicate_reservation",), len(pool), trace_id
            )
        gate = self._gate.evaluate(
            LifecycleOperation.SELECT_COMMENT_RESPONSE_TARGET, session_id, trace_id=trace_id
        )
        ...
        self._history.record(
            session_id, candidate.author_id, candidate.sanitized_text, candidate.message_type
        )
        self._stats[session_id] = replace(
            self.status(session_id), selected_count=self.status(session_id).selected_count + 1
        )
        self._publish(
    ...
    async def _features(
        ...
        if self._semantic is not None:
            try:
                ...
                scores = (semantic.relevance, semantic.conversation_fit, semantic.novelty)
    ...
    def _not_selected(
        ...
        self._publish(
            ...
            {"session_id": session_id, "reason_codes": reasons, "candidate_count": count},
            trace_id,
        )
    ...
    def _tokens(text: str) -> set[str]:
        return set(
            re.findall(r"[\wぁ-んァ-ヶ一-龠]{2,}", unicodedata.normalize("NFKC", text).casefold())
        )
