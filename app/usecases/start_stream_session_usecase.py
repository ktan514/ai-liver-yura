...
class StartStreamSessionUsecase:
    ...
    def validate(self, command: ApproveStreamStartCommand) -> None:
        ...
        if session.status not in {StreamSessionStatus.READY, StreamSessionStatus.START_FAILED}:
    ...
    async def _execute_once(self, command: ApproveStreamStartCommand) -> StreamStartResult:
        ...
        try:
            ...
            if (obs_status, stream_status, broadcast_status) != ("active", "active", "live"):
    ...
    def _event(self, event_type: str, command: ApproveStreamStartCommand, **data: object) -> None:
        self._publish(
            event_type,
            {"session_id": command.session_id, "command_id": command.command_id, **data},
            command.trace_id,
        )
