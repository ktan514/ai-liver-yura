...
class GameActivityFactory(Protocol):
...
class ShiritoriGameService:
    ...
    def __init__(
        ...
        *,
        emotion_provider: Callable[[], Mapping[str, object]] | None = None,
    ...
    def start_game(
        self,
        activity_manager: GameActivityFactory,
        *,
        started_by: ShiritoriPlayer = ShiritoriPlayer.AI,
        constraints: dict[str, object] | None = None,
    ) -> tuple[GameSession, PluginActivityWorkItem]:
    ...
    def submit_user_word(
        ...
        activity_manager: GameActivityFactory,
        value: str,
    ) -> tuple[ShiritoriWordResult, PluginActivityWorkItem]:
        ...
        if current is not None and current.game_type == "shiritori":
            ...
            if (
                isinstance(current_state, ShiritoriState)
                and current_state.end_reason is not None
            ):
        ...
        label = (
            ...
            in {ShiritoriValidation.VALID, ShiritoriValidation.ENDS_WITH_N}
    ...
    async def generate_ai_turn(
        self,
        activity: PluginActivityWorkItem,
        ...
        for retry_count in range(self._max_generation_attempts):
            ...
            if parsed is None:
                self._write_ai_rejected(
                    session, retry_count, "invalid_structured_output"
                )
                continue
    ...
    def parse_ai_output(raw_output: str) -> ShiritoriAiOutput | None:
        ...
        if game_action != "play_word":
            return None
        return ShiritoriAiOutput(
            game_action=game_action, word=word, utterance=utterance
        )
        ...
        *,
        ...
        context: dict[str, object] = {
            ...
            "activity_constraints": (
                dict(session.metadata.get("activity_constraints", {}))
    ...
    def _fallback_word(state: ShiritoriState) -> str | None:
        ...
        return next(
            (
                ...
                and (
                    state.expected_head is None or word.startswith(state.expected_head)
                )
            ),
            None,
        )

    def _write_ai_rejected(
        self, session: GameSession, retry_count: int, reason: str
    ) -> None:
        self._trace_logger.warning(
