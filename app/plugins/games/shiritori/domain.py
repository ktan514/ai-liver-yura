...
def validate_shiritori_word(
    ...
    *,
    ...
    if (
        state.expected_head is not None
        and get_shiritori_head(word) != state.expected_head
    ):
    ...
    next_player = (
        ShiritoriPlayer.AI if player == ShiritoriPlayer.USER else ShiritoriPlayer.USER
    )
    advanced = replace(
        state,
