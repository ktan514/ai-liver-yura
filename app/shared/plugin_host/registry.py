...
class PluginRegistry:
    ...
    def register(self, registration: PluginRegistration) -> None:
        ...
        exposed = (
            ...
            | set(registration.queries)
            | set(registration.activity_providers)
        )
        ...
        for item in registration.capability_registrations:
            ...
            if current and (
                ...
                or any(
                    existing.policy == CapabilityPolicy.SINGLE
                    for _, existing in current
                )
            ):
                raise DuplicateCapability(item.capability)
            if current and any(
                existing.policy != item.policy for _, existing in current
            ):
                raise DuplicateCapability(item.capability)
    ...
    def resolve_plugin(self, plugin_id: str) -> PluginRegistration:
        ...
        if registration is None or not registration.enabled:
            raise PluginUnavailable(
                f"Plugin is unavailable: {plugin_id}", plugin_id=plugin_id
            )
        return registration
    ...
    def resolve_command(
        ...
        return cast(
            CommandHandler[Any, Any],
            self._resolve_handler(capability, "commands", provider),
        )
    ...
    def resolve_query(
        ...
        return cast(
            QueryHandler[Any, Any],
            self._resolve_handler(capability, "queries", provider),
        )
