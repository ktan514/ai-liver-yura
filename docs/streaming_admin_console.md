...
背景は海上と海中を真横から見た断面構図とする。初期表示では、広い画面の上部状態カード左側に
確保した空間へ灯台と海面が収まる位置に海面を置く。灯台は海面へ追従しながら数pxの上下動と
小さな傾きを繰り返す。

海面は前後2層の波形を異なる速度で水平移動させる。画面下部へスクロールするほど海面と灯台を
上昇させ、ページ末尾では海面がほぼ画面上端へ到達する。操作部品の位置とクリック領域は動かさない。
...
概要にはサービスカード、復旧操作、折りたたみ式の操作分担を表示する。コメント、配信進行、
診断・ログにはそれぞれ独立したグリッドを与え、表やタイムラインは利用可能な横幅へ収める。
